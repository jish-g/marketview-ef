...
export default function NiftyTodayIndexPage() {
  const [dark, setDark] = useState(false)
  useEffect(() => { document.documentElement.classList.toggle('dark', dark) }, [dark])
  const supabase = createClient()

  const { data, error } = useSWR(['blog-posts', 'NIFTY'], async () => {
    const { data, error } = await supabase.from('blog_posts').select('*').eq('instrument', 'NIFTY').order('published_at', { ascending: false }).limit(60)
    ...
    <main className="blog-index-shell">
      ...
      <div className="blog-index-main">
        <div className="blog-index-head">
          <p className="eyebrow">Nifty Today</p>
          <h1 style={{ fontSize: 'clamp(26px,3vw,34px)', letterSpacing: '-.02em', marginTop: 8 }}>Nifty pre-market and post-market reads</h1>
          <p>Daily gap, PCR, Max Pain, and option-readiness reads for Nifty — published before the open and after the close, straight from the MarketCue rules engine.</p>
        </div>
        ...
        <div className="blog-post-list">
          {posts.map((post) => (
            <Link key={post.id} href={`/nifty-today/${post.slug}`} className="blog-post-card">
