...
import { Activity, AlertTriangle, ArrowDown, ArrowUp, BarChart3, BookOpen, CandlestickChart, CheckCircle2, ChevronRight, Clock3, Gauge, Globe, Info, Layers3, LogIn, LogOut, Menu, Moon, PenLine, RefreshCw, RotateCcw, Sigma, Sun } from 'lucide-react'
import { Line, LineChart, ReferenceLine, ResponsiveContainer, XAxis, YAxis } from 'recharts'
type Row = Record<string, string | number | boolean | null>
type Phase = 'premarket' | 'cues' | 'open' | 'verdict' | 'chart' | 'gex' | 'mid' | 'post' | 'journal' | 'rules' | 'history' | 'trade'
const PHASE_VALUES: Phase[] = ['premarket', 'cues', 'open', 'verdict', 'chart', 'gex', 'mid', 'post', 'journal', 'rules', 'history', 'trade']
const phases = [
...
// Same split for the Verdict answer line: bullish structures take the up colour, bearish the down, Iron Condor / No Trade stay in ink.
function strategyAnswerTone(strategy: string | null | undefined): string {
  const s = String(strategy ?? '')
  return BULLISH_STRUCTURES.includes(s) ? 'positive' : BEARISH_STRUCTURES.includes(s) ? 'negative' : ''
}
// No Trade reasoning closes with a manual-build hint; that hint lives under the Strategy override dropdown now, so drop it here.
const MANUAL_HINT = /\s*You can still build a position manually below\.?\s*$/i
function stripManualHint(reasoning: string, noTrade: boolean): string { return noTrade ? reasoning.replace(MANUAL_HINT, '').trim() : reasoning }
...
function VerdictInstrument({ row, instrument, agentOpen }: { row: Row; instrument: Instrument; agentOpen: AgentCall | null }) {
  ...
  // The row's own instrument gap (gap_points_<suffix> / prev_close_<suffix>) -- calc.open is NIFTY-only, which
  // printed a 0.00% SENSEX gap. Agent bias/strategy/confidence replace the rule-derived VIX phrasing.
  const suffix = instrument === 'NIFTY' ? 'nifty' : 'sensex'
  const summaryGapPts = rawNum(row, `gap_points_${suffix}`)
  ...
  return <article className="verdict-instrument">
    ...
    {strategy === 'No Trade' ? <div className="verdict-card no-trade-leg-builder"><p className="structure-line">No Trade is selected — there&apos;s no position to size. You can still build a position manually: switch the dropdown above to a real strategy.</p></div> : <>
...
export default function Dashboard() {
  ...
  // measured; syncing on that pass is what used to open the drawer on a phone for a frame.
