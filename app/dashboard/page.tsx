...
function PostMarketView({ row, postSummary }: { row: Row; postSummary: Row | null | undefined }) {
  ...
  return <section className="phase-view special-view verdict-view"><div className="review-section-head"><div><p className="eyebrow">Post market review · {syncLabel(postSummary, '15:45')}</p><h2>Close the loop</h2></div><span>Instrument outcomes</span></div><div className="verdict-instruments"><PostInstrumentCard row={row} postSummary={postSummary} instrument="NIFTY" /><PostInstrumentCard row={row} postSummary={postSummary} instrument="SENSEX" /></div><div className="flow-card"><span>FII / DII net cash flow</span><strong>{fii != null && dii != null ? `FII: ₹${fii} Cr, DII: ₹${dii} Cr (as of ${asOf ?? row.trade_date})` : 'Not available'}</strong></div></section>
}
...
function PhaseView({ phase, row, historyData }: { phase: Phase; row: Row | null; historyData?: HistoryExtras | null }) {
  ...
  const renderGroup = (title: string, group: typeof fields) => { const isIndex = title === 'Nifty' || title === 'Sensex'; const bias = title === 'Nifty' ? marketOpenBias('gap_points_nifty') : marketOpenBias('gap_points_sensex'); const summary = isIndex ? openSummary(title) : null; return <section className={`metric-group ${title === 'Common market data' ? 'common-group' : ''}`}><div className="group-heading">{title !== 'Common market data' && <h3>{title}{phase === 'open' && isIndex && <em className={`market-open-bias ${bias.toLowerCase()}`}> ({bias})</em>} {isIndex && <button type="button" className="semantic-info" aria-label={`${title} market color and OI action guidance`}><Info size={14} aria-hidden="true" /><span className="semantic-tooltip" role="tooltip"><b className="key-positive">Green</b> bullish / positive · <b className="key-negative">Red</b> bearish / negative · OI Addition = building interest · OI Unwinding = reducing interest</span></button>}</h3>}</div>{summary && <div className="verdict-banner prior-sessions-banner open-summary-banner"><p className="eyebrow">{title} at open</p><p className="prior-session-line">{summary}</p></div>}<div className="field-grid">{group.map((f) => { const isVix = f.key === 'india_vix'; const isOiAction = f.key.startsWith('oi_change_'); const actionTone = isOiAction ? (String(row?.[f.key] ?? '').toLowerCase() === 'addition' ? 'positive' : String(row?.[f.key] ?? '').toLowerCase().includes('unwinding') ? 'negative' : '') : ''; const relatedAction = f.key === 'oi_support_nifty' ? row?.oi_change_support_nifty : f.key === 'oi_support_sensex' ? row?.oi_change_support_sensex : f.key === 'oi_resistance_nifty' ? row?.oi_change_resistance_nifty : f.key === 'oi_resistance_sensex' ? row?.oi_change_resistance_sensex : null; const breadth = f.key === 'advance_decline_ratio' ? breadthDirection(row?.[f.key]) : null; const isNiftyGap = f.key === 'gap_points_nifty'; const predictedGap = isNiftyGap ? (row?.gift_nifty_gap_pts != null ? Number(row.gift_nifty_gap_pts) : row?.gift_nifty_gap_pct != null && row?.prev_close_nifty != null ? (num(row, 'gift_nifty_gap_pct') / 100) * num(row, 'prev_close_nifty') : null) : null; const prevCloseNifty = isNiftyGap && row?.prev_close_nifty != null ? Number(row.prev_close_nifty) : null; const gapDiff = isNiftyGap && predictedGap != null && row?.[f.key] != null && prevCloseNifty != null ? Number(row[f.key]) - predictedGap : null; const predictedOpenLevel = gapDiff != null ? prevCloseNifty! + predictedGap! : null; const actualOpenLevel = gapDiff != null ? prevCloseNifty! + Number(row![f.key]) : null; const fmtSigned = (n: number) => `${n > 0 ? '+' : ''}${n.toFixed(1)}`; const fmtLevel = (n: number) => n.toFixed(2); const isGiftGap = f.key === 'gift_nifty_gap_pct'; return <div className={`field-card ${row?.[f.key] == null ? 'is-empty' : ''}`} key={f.key}><span>{gapDiff != null ? 'Difference from Predicted' : f.label}</span><strong className={gapDiff != null ? (gapDiff > 0 ? 'positive' : gapDiff < 0 ? 'negative' : '') : tone(row, f.key)}>{gapDiff != null ? fmtSigned(gapDiff) : value(row, f.key, f.pct)}{isVix && row?.india_vix_change_pct != null && <em className={`vix-change ${tone(row, 'india_vix_change_pct')}`}> ({value(row, 'india_vix_change_pct', true)})</em>}{f.key.startsWith('prev_day_change_pct_') && row?.[f.key.replace('_pct_', '_pts_')] != null && <em className="prior-day-pts"> ({Number(row[f.key.replace('_pct_', '_pts_')]) > 0 ? '+' : ''}{row[f.key.replace('_pct_', '_pts_')]} pts)</em>}{isGiftGap && row?.gift_nifty_gap_pts != null && <em className="prior-day-pts"> ({Number(row.gift_nifty_gap_pts) > 0 ? '+' : ''}{row.gift_nifty_gap_pts} pts) expected open</em>}{relatedAction != null && <em className={`oi-action ${String(relatedAction).toLowerCase() === 'addition' ? 'positive' : String(relatedAction).toLowerCase().includes('unwinding') ? 'negative' : ''}`}> ({String(relatedAction)})</em>}{breadth && <em className={`breadth-flag ${breadth.tone}`}> {breadth.arrow} {breadth.label}</em>}{gapDiff != null && predictedOpenLevel != null && actualOpenLevel != null && <em className="predicted-gap">(Prediction from GIFT Nifty: {fmtLevel(predictedOpenLevel)}, Actual Open Nifty: {fmtLevel(actualOpenLevel)})</em>}</strong>{isOiAction && <small className={`oi-action ${actionTone}`}>{String(row?.[f.key] ?? '—')}</small>}{isGiftGap && giftRow?.last_price != null && row?.prev_close_nifty != null && <small className="gift-raw-compare">GIFT Nifty {giftRow.last_price} ({fmtTimeIST(giftRow.fetched_at)}) vs Nifty {row.prev_close_nifty} (Prev Close)</small>}</div> })}</div></section> }
