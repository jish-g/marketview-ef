...
export default function SensexTodayPostPage() {
  const params = useParams<{ slug: string }>()
  const [dark, setDark] = useState(false)
  useEffect(() => { document.documentElement.classList.toggle('dark', dark) }, [dark])
  const supabase = createClient()

  const { data, error } = useSWR(['blog-post', 'SENSEX', params.slug], async () => {
    const { data, error } = await supabase.from('blog_posts').select('*').eq('instrument', 'SENSEX').eq('slug', params.slug).maybeSingle()
  ...
  return (
    <main className="blog-post-shell">
      ...
      <div className="blog-post-main">
        <Link href="/sensex-today" className="blog-post-back"><ArrowLeft size={13} /> All Sensex Today posts</Link>
