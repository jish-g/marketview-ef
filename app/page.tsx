...
const phases = [
  { id: 'premarket' as Phase, label: 'Pre-market', icon: Clock3 },
  { id: 'open' as Phase, label: 'Market open', icon: Activity },
  { id: 'mid' as Phase, label: 'Mid-market', icon: Gauge },
  { id: 'post' as Phase, label: 'Post-market', icon: Layers3 },
]
...
export default function Dashboard() {
  ...
  return <main className="app-shell"><header className="topbar"><button className="icon-button mobile-menu" onClick={() => setNavOpen(!navOpen)} aria-label="Toggle navigation"><Menu size={18} /></button><div className="brand-mark"><div className="brand-symbol"><BarChart3 size={16} /></div><div><strong>MARKETVIEW</strong><span>TRADE ANALYSIS PLATFORM</span></div></div><div className="topbar-meta"><button className="icon-button" onClick={() => location.reload()} aria-label="Refresh dashboard"><RefreshCw size={16} /></button><button className="icon-button" onClick={() => setDark(!dark)} aria-label="Toggle theme">{dark ? <Sun size={16} /> : <Moon size={16} />}</button></div></header><div className="workspace"><aside className={`sidebar ${navOpen ? '' : 'closed'}`}><div className="nav-date"><strong>{row?.trade_date ?? 'No current row'}</strong><span>{row?.day_name ?? 'Session date'}</span></div><div className="side-label">SESSION MAP</div>{phases.map(({ id, label, icon: Icon }) => <button key={id} className={`phase-nav ${phase === id ? 'active' : ''}`} onClick={() => setPhase(id)}><Icon size={17} /><span><strong>{label}</strong></span><ChevronRight size={14} /></button>)}<div className="side-rule" /></aside><div className="content"><div className="content-head"><div><p className="eyebrow">{phases.find((item) => item.id === phase)?.label}</p></div></div>{phase === 'mid' ? <MidMarketView row={row} /> : phase === 'post' ? <PostMarketView row={row} /> : <PhaseView phase={phase} row={row} />}<div className="detailed-read-footer"><a className="detailed-read-link" href="/rules">Detailed read <ChevronRight size={15} /></a></div><footer className="data-footer"><span><CheckCircle2 size={14} /> Visual preview data</span><span>Snapshot: {row.trade_date}</span></footer></div></div></main>
}
