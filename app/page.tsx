...
function PhaseView({ phase, row }: { phase: Phase; row: Row | null }) {
  ...
  const renderGroup = (title: string, group: typeof fields) => <section className="metric-group"><div className="group-heading"><h3>{title}</h3><span>{group.filter((f) => row?.[f.key] != null && row?.[f.key] !== '').length}/{group.length}</span></div><div className="field-grid">{group.map((f) => <div className={`field-card ${row?.[f.key] == null ? 'is-empty' : ''}`} key={f.key}><span>{f.label}</span><strong className={tone(row, f.key)}>{value(row, f.key, f.pct)}</strong></div>)}</div></section>
  return <section className="phase-view"><div className="phase-context"><span>{available}/{fields.length} fields</span><span>{Math.round(available / fields.length * 100)}% coverage</span><span>{intros[phase][0]}</span></div><div className="interpretation"><BookOpen size={18} /><div><strong>How to interpret this phase</strong><p>{phase === 'premarket' ? 'Shared volatility and overnight context come first. Then compare Nifty and Sensex levels and positioning independently.' : phase === 'open' ? 'Read common breadth first, then compare the Nifty and Sensex opening response rather than blending them.' : phase === 'mid' ? 'Use common session context to frame the read; check each index for confirmation or divergence.' : 'Review the shared session outcome, then evaluate Nifty and Sensex closes separately.'}</p></div></div><div className="metric-groups">{renderGroup('Common market data', common)}{renderGroup('Nifty', nifty)}{renderGroup('Sensex', sensex)}</div></section>
}
