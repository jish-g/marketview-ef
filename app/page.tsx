...
function VerdictView() { const fields = [['Market Bias', 'Calculated from opening data'], ['Suggested Strategy', 'Auto-selected after Market Open'], ['Confidence Score', 'Calculated confidence'], ['Risk Mode', 'Calculated risk profile']]; return <section className="phase-view verdict-view"><div className="review-section-head"><div><p className="eyebrow">After Market Open</p><h2>Verdict</h2></div><span>Calculated strategy</span></div><div className="verdict-banner"><div><p className="eyebrow">Auto-generated read</p><strong>Strategy selection will appear here</strong><p>These values are calculated from the Market Open snapshot. They are read-only and remain blank until the calculation is available.</p></div><CheckCircle2 size={20} /></div><div className="verdict-grid">{fields.map(([label, hint]) => <div className="verdict-card" key={label}><span>{label}</span><strong>—</strong><small>{hint}</small></div>)}</div><div className="verdict-rationale"><span>Rationale</span><p>Not yet available</p></div></section> }
